...
6. Write the pages into /workspace/repo/${pseoConfig.targetDir} with write_file following the programmatic-seo quality bar, then publish once with publish_seo_pages: confirmPublish=true, branch pseo/<year>-w<ISO week> derived from today's date so a replayed run reuses the same branch and pull request, and pagePaths listing the written files. Never run git push from the sandbox, and never merge the pull request.
