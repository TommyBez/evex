...
import { missingWatchConfig, watchConfig } from "../lib/watch-config.js";
import { storeKind } from "../lib/snapshot-store.js";

export default defineTool({
  description:
    "Load the competitor watch list, cron, alert thresholds, store kind, and which delivery targets are configured. Does not return webhook URLs, API keys, or other secrets. Call this first on a scheduled run.",
  ...
  execute() {
    ...
    return {
      ...
      delivery: {
        slackConfigured: Boolean(watchConfig.slackWebhookUrl),
