...
4. For each successful fetch, call `diff_page_snapshot` with the tool's `text`, `hash`, and `fetchedAt`. The tool writes the new snapshot to the store. A first-seen URL is a baseline (`isBaseline: true`) and is not an alert.
...
8. To send for real, call `send_digest` with `confirmSend: true` and a stable `idempotencyKey` derived from the run date (for example `competitor-intel-monitor-YYYY-MM-DD`). Never call `send_digest` without an idempotency key. Reuse the same key if the step is retried. If the tool returns `sent: false` with an `error`, report that the digest was not delivered and do not retry in the same run.
