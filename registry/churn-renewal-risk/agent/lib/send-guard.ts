...
export function assertNeverEmailCustomer(url: string, method = "GET"): void {
  if (isForbiddenSendUrl(url)) {
    throw new Error(
      `Refused ${method} ${url}: this agent never emails the customer. Slack is the only outbound notify, and only for the movers digest.`,
    );
  }
}
...
export function assertNotEmailIntent(intent: string | undefined): void {
  ...
  if (
    ...
    throw new Error(
      "Refused email intent. This agent never auto-emails the customer.",
    );
  }
}
